...
:root{
  ...
  /* ⚠️ Gövde kenarları belirgin olmalı; açık tonlar greige zeminde kayboluyor. */
  --dg-kenar:#AFA698;      /* açık buton */
  --dg-kenar-koyu:#111309; /* koyu buton */
  --koyu-1:#41443E;        /* koyu buton üst */
  --koyu-2:#212420;        /* koyu buton alt */
...
.dg{
  ...
  border:2px solid #2B2D29;
  /* ⚠️ Saydam zemin greige üzerinde soluk kalıyordu — dock ve bildirimle aynı kural: NET BEYAZ. */
  background:linear-gradient(180deg,#fff 0%,#F1EEE8 100%); color:#242622;
  box-shadow:
    inset 0 1px 0 rgba(255,255,255,1),
    inset 0 -4px 0 rgba(59,60,57,.10),
    0 6px 0 var(--dg-kenar),
    0 14px 24px rgba(59,60,57,.28);
  transform:translateY(0);
  transition:background .22s,color .22s,transform .14s cubic-bezier(.2,.8,.3,1),box-shadow .14s;
}
.dg:hover{
  background:linear-gradient(180deg,#fff 0%,#E9E5DD 100%);
  transform:translateY(-3px);
  box-shadow:
    inset 0 1px 0 rgba(255,255,255,1),
    inset 0 -4px 0 rgba(59,60,57,.10),
    0 9px 0 var(--dg-kenar),
    0 20px 32px rgba(59,60,57,.34);
}
.dg:active{
...
.dg-koyu{
  background:linear-gradient(180deg,var(--koyu-1) 0%,var(--koyu-2) 100%); color:#fff;
  border-color:#15170F;
  box-shadow:
    inset 0 1px 0 rgba(255,255,255,.22),
    inset 0 -4px 0 rgba(0,0,0,.3),
    0 6px 0 var(--dg-kenar-koyu),
    0 14px 26px rgba(33,36,32,.45);
}
.dg-koyu:hover{
  background:linear-gradient(180deg,#4C4F49 0%,#2A2D28 100%); color:#fff;
  box-shadow:
    inset 0 1px 0 rgba(255,255,255,.26),
    inset 0 -4px 0 rgba(0,0,0,.3),
    0 9px 0 var(--dg-kenar-koyu),
    0 20px 34px rgba(33,36,32,.5);
}
.dg-ara{
...
.alt-kart{
  /* Genişlik içeriğe göre: en uzun satır (adres) + boşluk + Instagram rozeti.
     Daha geniş bırakılırsa ortada ölü alan kalıyor. */
  background:var(--kart); max-width:880px; margin:0 auto;
  padding:clamp(34px,5vw,52px) clamp(24px,4vw,56px);
  /* Bilgiler solda, Instagram sağda. `flex-end` sayesinde rozet, sol bloğun SON satırı olan
     telefon numarasıyla aynı hizaya oturuyor. */
  display:flex; align-items:flex-end; justify-content:space-between; gap:32px;
}
.alt-bilgi{min-width:0}
.alt-logo{margin:0 0 4px; color:var(--kurum)}
.alt-kisi{font-weight:600; margin:0 0 14px}
.alt-sat{margin:0 0 6px; color:var(--murekkep)}
.alt-tel{margin-bottom:0}
.alt-sat a{text-decoration:none; border-bottom:1px solid var(--hat)}
.alt-sos{margin:0; flex:0 0 auto}
.sos{
...
@media (max-width:720px){
  .alt-kart{flex-direction:column; align-items:flex-start; gap:22px}
}
...
/* ⚠️ Pay dock YÜKSEKLİĞİNDEN fazla olmalı, yoksa imza dock'un altında kalıp tıklanamaz.
   Ölçülen: ≤520px'te dock 180px + 12px kenar; 760px'e kadar 190px + 18px kenar.
   Düğme sayısı/boyutu değişirse tarayıcıda yeniden ölç, tahmin etme. */
@media (max-width:760px){ .alt-serit{padding-bottom:230px} }
@media (max-width:520px){ .alt-serit{padding-bottom:214px} }
...
.dock-wa{--renk:#25D366}
/* Instagram dairesi düz renk değil, markanın köşegen degradesi */
.dock-ig .dock-ikon{
